...
export class FlowViewSelector extends FlowViewBase {
	...
	handleEvent(event) {
		...
		if (event.type === "keyup") {
				...
				switch (this.matchingNodes.length) {
					...
					default:
						if (this.highlightedOptionIndex === -1) {
							...
							const shortestMatch = this.matchingNodes.reduce(
								(shortest, match) =>
									shortest.length < match.length
										? shortest
										: match
							)

							for (
								let i = this.input.value.length;
								i < shortestMatch.length;
								i++
							) {
								const currentChar = shortestMatch[i]
								if (
									this.matchingNodes.every((name) =>
										name.startsWith(
			...
			switch (event.code) {
				...
				case "ArrowRight":
					if (
						this.input.value.length === event.target.selectionStart
					) {
						autocomplete()
						resetOptions()
					}
	...
	set position({ x, y }) {
		...
		const overflowX =
			x - view.origin.x + FlowViewSelector.width >= view.width
		const _x = overflowX
	...
	createNode() {
		const nodeText =
			this.options?.children?.[this.highlightedOptionIndex]
				?.textContent ?? this.input.value
		const matchingNodeText = this.nodeNames.find(
