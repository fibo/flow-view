...
type SVGElementWithSet = SVGElement & {
	set(attributeName: string, value: string): SVGElementWithSet
}
 ...
 *   .set('height', '100')
 */
function createElementSvg(qualifiedName: string): SVGElementWithSet  {
	...
	} as SVGElementWithSet)
}

/** Look for the first parent element with the given name containing the element. */
function findParentElement<Elem>(parentElementName: string, initialElement: Element) {
	let { parentElement: element } = initialElement
	while (element) {
		if (element.localName == parentElementName)
			return element as Elem
		element = element.parentElement
	}
	throw new Error('Parent element not found', { cause: { parentElementName, initialElement } })
}
 ...
 * `
 */
function html(strings: TemplateStringsArray, ...expressions: string[]): HTMLTemplateElement {
...
const cssSheet = (rows: string[]): CSSStyleSheet => {
	const sheet = new CSSStyleSheet()
	sheet.insertRule(rows.join(''))
	return sheet
}

const cssBlock = (selector: string, rows: string[]): string[] =>
	[`${selector} {`, rows.join(';'), '}']

/** Calculates the coordinates of a pointer event, relative to a DOM element. */
function pointerCoordinates({ clientX, clientY }: PointerEvent, { left, top }: DOMRect): Vector {
	return {
...
const styleSheets: Record<FVCustomElementName, CSSStyleSheet[]> = {
	'v-canvas': [
		cssSheet(cssBlock(':host', [
			...
			'font-family: var(--flow-view-font-family, system-ui, Roboto, sans-serif)',
			...
			...cssBlock('@media (prefers-color-scheme: dark)', [
...
class UidRegister {
	...
	#newUid(len = 2) {
		...
		while (alreadyExists) {
			uid = Math.random().toString(36).substring(2, 2 + len)
...
class VCanvas extends HTMLElement {
	...
	#resizeObserver = new ResizeObserver((entries) => {
		for (const { contentBoxSize: [{ blockSize, inlineSize }] } of entries.filter(
	...
	constructor() {
		...
		this.shadowRoot!.adoptedStyleSheets = styleSheets['v-canvas']
...
class VEdge extends HTMLElement {
	...
	updateBoundingRect() {
		...
		const width = x2 - x1
		const height = y2 - y1
		this.#boundingRect = { top: y1, left: x1, width, height }
		this.#setCssProps()
...
export function defineFlowViewCustomElements() {
	...
	customElements.define('v-node', VNode)
	customElements.define('v-pin', VPin)
