...
export class FlowViewSelector extends FlowViewBase {
	...
	get matchingNodes() {
		...
		return this.nodeNames.filter(
			(name) => name.toLowerCase().startsWith(search) && name.toLowerCase() !== search,
		);
	}
	set position({ x, y }) {
				...
				default:
					if (this.highlightedOptionIndex === -1) {
						...
						const shortestMatch = this.matchingNodes.reduce(
							(shortest, match) => shortest.length < match.length ? shortest : match,
						);
	...
	onKeydown(event) {
		event.stopPropagation();
		if (["ArrowUp", "Tab"].includes(event.code)) {
			event.preventDefault();
		}
	}
	onPointerdown(event) {
