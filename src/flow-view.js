...
function pointerCoordinates({ clientX, clientY }, { left, top }) {
	return {
...
class VCanvas extends HTMLElement {
	...
	#isValidUnit(value) { return value > 1 && value < 25 }
...
class VPin extends HTMLElement {
	...
	connectedCallback() {
		...
		const normalizedUid = normalizeUid(uidValue.trim())
...
class VEdge extends HTMLElement {
	...
	attributeChangedCallback( name, _oldValue, newValue) {
		if (name === 'path') {
			...
			const uids = newValue.split(',').map(normalizeUid.trim())
